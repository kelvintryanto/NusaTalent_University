        ...
        $.ajax({
            ...
            success: function (dataEvent) {
                ...
                $.each(dataEvent, function (idx, result) {
                    if(result.company == null) result.company = "No";
                    let content = '<a type="button" href="/event/' + result.id + '" ><div class="panel-list"><div style="margin-right: 60px;"><img src="../images/nusatalent.png" class="avatar-listjob"></div><div style="display: flex; flex-direction: column; width: 100%;"><div style="display: flex; justify-content: space-between; width: 95%;"><div><label class="panel-title">' + result.name + '</label></div><div><object><a type="button" href="/event/'+result.id+'/editEvent"><i class="fas fa-edit fa-lg" style="color: #04518D; margin-right:20px;"></i></a><a type="button" data-toggle="modal" data-target="#deleteCompany"><i class="far fa-trash-alt fa-lg" style="color: #04518D;"></i></a></object></div></div><div style="display: flex; width: 57%;"><div><i class="fas fa-map-marker-alt fa-lg icon"></i><object><a class="img-icon panel-subtitle text2" style="margin-top: 10px;">Location (Lokasi diambil dari career_location table, bikin baru)</a></object></div></div><div style="display: flex; justify-content: space-between; width: 80%; margin-top: 20px;"><div><label class="text13">'+result.company+'</label><label class="text13">&nbsp;Company</label></div><div><label class="text13">75</label><label class="text13">&nbsp;Job Post(s)</label></div><div><label class="text13">800</label><label class="text13">&nbsp;Visitor(s)</label></div></div></div></div></a>'
